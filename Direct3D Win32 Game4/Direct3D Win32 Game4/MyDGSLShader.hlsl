...
P2F main(V2P pixel)
{
    ...
    float3 local2 = SpecularContribution(tangentToEyeVec, tangentLightDir, float3(0.000000f,0.000000f,1.000000f), MaterialSpecular.rgb, MaterialSpecularPower, LightSpecularIntensity[0], LightColor[0].rgb);
    float3 local3 = mad(0.200000f, CubeTexture2.Sample(TexSampler, normalize(mul(worldToTangent, reflect(-tangentToEyeVec, float3(0,0,1))))).rgb, local2);
    float3 local6 = LambertLighting(tangentLightDir, float3(0.000000f,0.000000f,1.000000f), MaterialAmbient.rgb, AmbientLight.rgb, LightColor[0].rgb, Texture1.Sample(TexSampler, pixel.uv).rgb);
    float3 local7 = local3 + local6;
    result.fragment = CombineRGBWithAlpha(local7, Texture1.Sample(TexSampler, pixel.uv).a);
    // END GENERATED CODE
